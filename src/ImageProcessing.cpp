...
void ImageProcessing::classify( int* data ) {
	...
	for ( int i = 0; i < 320*240; i++ ) {
		...
		if ( r > (g+b)*5/8 & r > g*14/10 ) {		//RED
			...
			data[i] = 0xff00ff00;
		} else if ( b > (r+g)*9/16 & r > g & dist>3*50*50) {		//PURPLE
			data[i] = 0xffff00ff;	
		} else if ( b > (r+g)*5/8 ) {		//BLUE
			data[i] = 0xffff0000;
		} else if ( dist<3*120*120 ) {	//FLOOR
			data[i] = 0xffff0000;
		} else {
			data[i] = 0xff000000;
		}
			...
			if ( pix==0xff0000ff | pix==0xff00ff00 ) {
				...
				if ( valid ) {
					...
					if ( Xcoord>10 && Xcoord<310 && Zcoord>10 && Zcoord<230 ) {
						for ( float ext = 0; ext<=1.0; ext+=0.01f ) {
							int Xcoord = (int)((X*ext)*scale_map+160);
							int Zcoord = (int)(-(Z*ext)*scale_map+120);
							int dest = Xcoord+Zcoord*320;
							for ( int xx = -8; xx<=8; xx++ ) {
								if ( map[dest+xx]&0x00ffffff!=0x00000000 ) {
									unreachable++;
									if ( unreachable>15 ) {
										ext=1.1;
										break;
